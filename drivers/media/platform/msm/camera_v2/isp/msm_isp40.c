...
static void msm_vfe40_init_qos_parms(struct vfe_device *vfe_dev)
{
	...
	if (vfe_dev->vfe_hw_version == VFE40_V1_VERSION) {
		...
		msm_camera_io_w(0x0002AAAA, vfebase + VFE40_BUS_BDG_QOS_CFG_7);
		   ...
		   vfe_dev->vfe_hw_version == VFE40_V3_VERSION) {
...
static void msm_vfe40_process_error_status(struct vfe_device *vfe_dev)
{
	...
	if (error_status1 & (1 << 0)) {
		...
		error_event.input_src = VFE_PIX_0;
		error_event.u.error_info.error_mask = (1 << ISP_CAMIF_ERROR);
	...
	struct vfe_device *vfe_dev)
{
	...
	prop_size = MSM_ISP40_TOTAL_WM_UB -
		    ...
		    - axi_data->hw_info->extra_ub * axi_data->num_rdi_stream;
	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		if (axi_data->free_wm[i]) {
			...
			if (axi_data->rdi_wm_mask & (1 << i)) {
				pr_err("%s harsh adding %d UB to WM %d \n", __func__,
				       axi_data->hw_info->extra_ub, i);
				wm_ub_size += axi_data->hw_info->extra_ub;
			}
			      ...
			      uint32_t blocking)
{
	...
	if (vfe_dev->axi_data.stream_update) {
		pr_err("%s harsh calling complete on stream update\n", __func__);
